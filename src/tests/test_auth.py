...
def test_register_user(test_engine):
...
def test_login_user(test_engine):
    auth = Auth(secret_key=SECRET_KEY)
    token = auth.login_user("test@example.com", "password123")
    assert isinstance(token, str)


def test_register_existing_user(test_engine):
    auth = Auth(secret_key=SECRET_KEY)
    with pytest.raises(AuthenticationError):
        auth.register_user("test@example.com", "newpassword")


def test_login_invalid_credentials(test_engine):
    auth = Auth(secret_key=SECRET_KEY)
